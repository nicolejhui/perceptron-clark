...
class Node:
    ...
    def get_entropy(self):
        label, label_counts = np.unique(all_examples[:, 0], return_counts=True)
        # print(label, label_counts)
        label1_prob = label_counts[0] / all_examples.shape[0]
        label2_prob = label_counts[1] / all_examples.shape[0]
    ...
    def con_entropy(self, attribute_col):
        ...
        print('example data used:', str(examples))
        ...
        for unique_var, counts, in conditional_array.items():
            ...
            frequency = (counts[0] + counts[1]) / examples.shape[0]
    ...
    def find_best_attribute(self, examples):
        # define entropy
        ...
        best_attribute_col = examples[:, best_attribute_index]
        ...
        print('-----------------------------------NEXT SPLIT--------------------------------------')
        return examples, best_attribute_name, best_attribute_index, best_attribute_col, ig_list, info_gain_dict, conditional_entropy

    def split_node(self):
        # get best split
        examples, best_attribute_name, best_idx, best_attribute, ig_list, info_gain_dict, conditional_entropy = self.find_best_attribute(split)
        ...
        for unique_val in np.unique(best_attribute):
            ...
            child = Node(unique_val_of_best_attribute[unique_val])
            ...
            if info_gain_dict[best_attribute_name] < 0.001:
                ...
                return
            child.split_node()
            child.children[unique_val_of_best_attribute[unique_val]] = child
            return split
